      ...
      {regions.length > 0 ? (
        regions.map((region) => (
          <button
            ...
            className={`px-4 sm:px-6 md:px-10 py-2 w-24 rounded-lg text-sm sm:text-base ${
        ...
        {categories.slice(currentIndex, currentIndex + maxImages).map((cat) => (
          <div key={cat.id} className="">
            <button
              ...
              className={`border rounded-md p-4 border-[#4A62AA] flex items-center w-28 sm:w-44 justify-center h-[80px] text-sm ${
      ...
      <div className="w-full overflow-x-auto">
        {rankings.length > 0 ? (
          <table className="w-full sm:w-[80%] text-center border-collapse">
            <thead>
              <tr>
                <th className="border-[#A87A38] p-2 text-xs sm:text-base">Université</th>
                <th className="border-[#A87A38] p-2 text-xs sm:text-base">Rang</th>
            ...
            <tbody>
              {rankings.map((uni, index) => (
                <tr key={index} className="text-center">
                  <td className={`border border-x-0 text-start text-black border-[#A87A38] ${
                    index % 2 === 0 ? 'bg-white' : 'bg-[#f0f0f0]'
                  } p-2 text-xs sm:text-base`}>
                    {uni.institution}
                  </td>
                  <td className={`border border-x-0 border-[#A87A38] ${
                    index % 2 === 0 ? 'bg-white' : 'bg-[#f0f0f0]'
                  } p-2 text-xs sm:text-base`}>
                    {uni.rank}
                  </td>
                  <td className={`border border-x-0 border-[#A87A38] ${
                    index % 2 === 0 ? 'bg-[#c7d0e9]' : 'bg-[#d7e0f9]'
                  } p-2 text-xs sm:text-base`}>
          ...
          <div className="text-center w-full py-8 text-gray-500">
            {selectedCategory
              ? `Aucun classement disponible pour ${selectedCategory.label}`
              : ""}
          </div>
        )}
      </div>
